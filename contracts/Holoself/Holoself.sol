    ...
    ReentrancyGuard
{
        ...
        ERC721A("Holoself", "Holo")
    {
        ...
        _setDefaultRoyalty(0x11C4c9AcC91cA75f3dce47B5b13506F5A8A00000, 500);
    }
    ...
    /**
     ** @dev ETHを受け取り、NFTをミントする。
     ** @param _quantity ミントする量
     */
    ...
    {
        // 公開セールがアクティブでない場合は失敗する
        if (!publicSaleActive) revert SaleNotActive();

        // トータルサプライが最大サプライを超える場合は失敗する
    ...
    /**
     ** @notice 「presaleMint」関数は、1つのウォレットに対してのみ単一のトランザクションを受け付けます。
     ** it also expects a byte32 slice as calldata to provide valid proof of list.
     ** @dev ETHを受け取り、NFTをミントする。
     ** @param _merkleProof byte32[]形式のMerkleProof
     ** @param _quantity ミントする量
     */
    function presaleMint(bytes32[] calldata _merkleProof, uint256 quantity)
        ...
        requireCorrectEth(quantity)
    {
        // プレセールがアクティブでない場合は失敗する
        if (!preSaleActive) revert SaleNotActive();

        // ユーザーがプレセールに参加していない場合は失敗する
